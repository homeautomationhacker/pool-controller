...
//void turnOn(char *deviceId);
//void turnOff(char *deviceId);
...
void setup() {
  ...
  // All the auxiliary pins are output pins that are used to fire an optocoupler
  pinMode(AUX_1, OUTPUT);
  pinMode(AUX_2, OUTPUT);
  pinMode(AUX_3, OUTPUT);
  pinMode(AUX_4, OUTPUT);
...
void webSocketEventCallback(WStype_t type, uint8_t *payload, size_t length) {
  switch(type)
  {
    case WStype_DISCONNECTED:
      isConnected = false;
      DEBUG_PRINTLN("[WebSocketCallback] Webservice disconnected from " + SINRIC_HOST);
      break;
    case WStype_CONNECTED:
      isConnected = true;
      DEBUG_PRINTLN("[WebSocketCallback] Webservice connected to " + SINRIC_HOST);
      DEBUG_PRINLN("Waiting for commands from " + SINRIC_HOST);
      break;
    case WStype_TEXT:
  ...
  // TODO: Used later
  // const int JSON_BUFFER_SIZE = JSON_OBJECT_SIZE(3);
